...
import { FightStatusResultStruct } from './Structs/FightStatusResultStruct';
...
export abstract class FightPackets {
	...
	/**
	 * Updates fight member status without an action animation.
	 * @param results
	 */
	public static status(results: { id: number; status: number; hp: number }[]): Packet {
		let count = results.length;
		let packet = new Packet(
			16 + count * FightStatusResultStruct.size,
			PacketType.FightStatus,
		).uint32(12, count);
		let offset = 16;

		for (const result of results) {
			packet.struct(offset, FightStatusResultStruct, result);
			offset += FightStatusResultStruct.size;
		}
