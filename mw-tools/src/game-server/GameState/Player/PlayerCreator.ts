...
import type { BaseStats, StatRates } from '../Stats/StatRates';
...
export abstract class PlayerCreator {
	...
	private static createStats(race: CharacterRace): StatsJson {
		...
		let base = this.getBaseStats(race);
	...
	private static getBaseStats(race: CharacterRace): BaseStats {
		switch (race) {
			case CharacterRace.Human:
				return { sta: 15, int: 1, str: 1, agi: 8 };
			case CharacterRace.Centaur:
				return { sta: 15, int: 1, str: 1, agi: 8 };
			case CharacterRace.Mage:
				return { sta: 1, int: 15, str: 1, agi: 8 };
			case CharacterRace.Borg:
				return { sta: 1, int: 1, str: 15, agi: 8 };
		}
	}
