...
export class Stats {
	...
	public autoLevelsApplied: number;
	...
	public constructor(json: StatsJson) {
		...
		this.autoLevelsApplied = json.autoLevelsApplied ?? 0;
	}
	...
	public toJson(): StatsJson {
		return {
			...
			unused: this.unused,
			autoLevelsApplied: this.autoLevelsApplied,
		};
	}
	 ...
	 * @param hp
	 */
	public addHp(hp: number, maxHp?: number): void {
		this.currentHp = Math.max(0, Math.min(this.currentHp + hp, maxHp ?? this.hp.stat));
	}
	 ...
	 * @param hp
	 */
	public addMp(mp: number, maxMp?: number): void {
		this.currentMp = Math.max(0, Math.min(this.currentMp + mp, maxMp ?? this.mp.stat));
	}
	 ...
	 */
	public addHpPerc(value: number, maxHp?: number): void {
		this.addHp(((maxHp ?? this.hp.stat) * value) / 100, maxHp);
	}
	 ...
	 * @param value Value from 0 and 100
	 */
	public addMpPerc(value: number, maxMp?: number): void {
		this.addMp(((maxMp ?? this.mp.stat) * value) / 100, maxMp);
	}
