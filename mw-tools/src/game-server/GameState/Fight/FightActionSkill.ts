...
export class FightActionSkill {
	...
	public static execute(action: FightAction, skillData: SkillsGroup): FightActionResult {
		...
		case SkillType.Blizzard:
			targets.forEach(target =>
				this.executeMagicSkill(result, action.resolveDefendTarget(target), skillData),
			);
				break;
		...
		case SkillType.Drain:
			targets.forEach(target =>
				this.executeDrainSkill(result, action.resolveDefendTarget(target), skillData),
			);
				break;
		}
	...
	private static executeEffectSkill(
		...
		if (skill.type === SkillType.Poison) {
			...
			const maxHp = target.base.fightStats.totals.hp;
			const damage = Math.floor((maxHp * poisonPercent) / 100);
			target.base.fightData.stats.addHp(-damage, maxHp);
			if (target.effect.has(FightEffect.Hypnotize)) {
				target.effect.remove(FightEffect.Hypnotize);
				target.effectCounters.set(FightEffect.Hypnotize, 0);
			}
	...
	private static executeHealSkill(
		...
		// Rank 3: 40% (single)
		// Rank 4: 25% (multi)
		if (skill.rank === 2) basePercent = 15;
		else if (skill.rank === 3) basePercent = 40;
		else if (skill.rank === 4) basePercent = 25;

		const levelBonus = (skillData.level - 1) * 2; // Each level adds 2%
		const healPercent = basePercent + levelBonus;

		const maxHp = target.base.fightStats.totals.hp;
		const healing = Math.floor((maxHp * healPercent) / 100);
		//revive if dead
		if (target.base.fightData.stats.currentHp === 0) {
			target.base.fightData.stats.addHp(healing, maxHp);
	...
	private static executeDrainSkill(
		...
		const maxHp = target.base.fightStats.totals.hp;
		const maxMp = target.base.fightStats.totals.mp;

		// Calculate HP and MP drain
		const hpDrain = Math.floor((maxHp * drainPercent) / 100);
		const mpDrain = Math.floor((maxMp * drainPercent) / 100);

		// Apply drains
		target.base.fightData.stats.addHp(-hpDrain, maxHp);
		target.base.fightData.stats.addMp(-mpDrain, maxMp);
