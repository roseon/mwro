...
export class FightMember {
	...
	/**
	 * Whether the member has received an action this turn.
	 */
	public actionReceived: boolean = false;
	...
	public isDead(): boolean {
		...
		if (dead && !this.effect.has(FightEffect.Dead)) {
			if (this.base instanceof Pet) {
				for (const player of this.fight.players) {
					if (player.pets.some(pet => pet.id === this.base.id)) {
						this.base.loyalty = Math.max(0, this.base.loyalty - 10);
						player.client?.write(PetPackets.list(player.pets));
						break;
					}
				}
			}
