...
export class SkillTargeting {
	...
	public static getTargets(action: FightAction, sort: 'speed' | 'hp'): FightMember[] {
		...
		let skillData: SkillsGroup | undefined = action.member.base.fightData.skills.skillData.find(
			s => s.id === action.detail,
		);

		if (!skillData) {
			// Create a temporary skill data if the character doesn't have the skill
			// This can happen for testing or if the skill was removed
			skillData = new SkillsGroup(action.detail);
		}
